...
class Base(nn.Module):
    def __init__(self):
        ...
        self.mean = nn.Parameter(torch.Tensor(
            rgb_mean).view(1, 3, 1, 1), requires_grad=False)
        rgb_std = (0.229, 0.224, 0.225)
        self.std = nn.Parameter(torch.Tensor(
            rgb_std).view(1, 3, 1, 1), requires_grad=False)


class CL2SNet(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        super(CL2SNet, self).__init__()
        ...
        self.t = nn.Sequential(
            nn.Conv2d(num_features, num_features // 2,
                      kernel_size=3, padding=1), nn.SELU(),
    ...
    def forward(self, x0, x0_hd=None):
        ...
        f_phy = down1 * attention_phy[:, 0, :, :, :] + down2 * attention_phy[:, 1, :, :, :] + \
            down3 * attention_phy[:, 2, :, :, :] + \
            down4 * attention_phy[:, 3, :, :, :]
        ...
        log_log_x0_inverse = torch.log(
            torch.log(1 / x0.clamp(min=1e-8, max=(1 - 1e-8))))
        ...
        x_j3 = torch.exp(-torch.exp(log_log_x0_inverse + r3)
                         ).clamp(min=0., max=1.)

        # J4 = ( sin(I + R4) + 1 ) / 2
        r4 = F.upsample(self.j4(f4), size=x0.size()[2:], mode='bilinear')
        x_j4 = (torch.sin(x0 + r4)+1)/2

        attention_fusion = F.upsample(self.attention_fusion(
            concat), size=x0.size()[2:], mode='bilinear')
...
class Base_OHAZE(nn.Module):
    def __init__(self):
        ...
        self.std_in = nn.Parameter(torch.Tensor(
            rgb_std).view(1, 3, 1, 1), requires_grad=False)

        rgb_mean = (0.35851, 0.35316, 0.34425)
        self.mean_out = nn.Parameter(torch.Tensor(
            rgb_mean).view(1, 3, 1, 1), requires_grad=False)
        rgb_std = (0.16391, 0.16174, 0.17148)
        self.std_out = nn.Parameter(torch.Tensor(
            rgb_std).view(1, 3, 1, 1), requires_grad=False)


class CL2SNet_woPhy(Base_OHAZE):
    def __init__(self, num_features=64, arch='resnext101_32x8d'):
        super(CL2SNet_woPhy, self).__init__()
        self.num_features = num_features
    ...
    def forward(self, x0):
        ...
        x_p0 = torch.exp(log_x0 + F.upsample(self.p0(f),
                         size=x0.size()[2:], mode='bilinear')).clamp(min=0, max=1)

        x_p1 = ((x + F.upsample(self.p1(f), size=x0.size()[2:], mode='bilinear')) * self.std_out + self.mean_out)\
            .clamp(min=0., max=1.)

        x_p2 = torch.sin(x0 + F.upsample(self.p2(f),
                         size=x0.size()[2:], mode='bilinear')) / 2 + 0.5

        log_x_p3_0 = torch.exp(
            log_log_x0_inverse + F.upsample(self.p3_0(f), size=x0.size()[2:], mode='bilinear'))
        x_p3 = torch.exp(-log_x_p3_0 + F.upsample(self.p3_1(f), size=x0.size()[2:], mode='bilinear')).clamp(min=0,
                                                                                                            max=1)

        attention_fusion = F.upsample(self.attentional_fusion(
            f), size=x0.size()[2:], mode='bilinear')
