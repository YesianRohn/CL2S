...
from model import CL2SNet, CL2SNet_woPhy
        ...
        for name, root in to_test.items():
            ...
            if len(args['snapshot']) > 0:
                ...
                net.load_state_dict(torch.load(os.path.join(
                    ckpt_path, exp_name, args['snapshot'] + '.pth')))
            ...
            print(
                f"[{name}] L1: {loss_record.avg:.6f}, PSNR: {np.mean(psnrs):.6f}, SSIM: {np.mean(ssims):.6f}")
