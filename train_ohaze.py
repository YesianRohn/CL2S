...
from model import CL2SNet_woPhy
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train a CL2SNet')
    parser.add_argument(
        '--gpus', type=str, default='0', help='gpus to use ')
    parser.add_argument('--ckpt-path', default='./ckpt',
...
def validate(net, curr_iter, optimizer):
    ...
    with torch.no_grad():
        for data in tqdm(val_loader):
            ...
            for i in range(len(haze)):
                r = dehaze[i].cpu().numpy().transpose(
                    [1, 2, 0])  # data range [0, 1]
    ...
    snapshot_name = 'iter_%d_loss_%.5f_lr_%.6f' % (
        curr_iter + 1, loss_record.avg, optimizer.param_groups[1]['lr'])
